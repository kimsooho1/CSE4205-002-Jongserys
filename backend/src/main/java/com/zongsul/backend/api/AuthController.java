...
public record RegisterRequest(@NotBlank String userId, @NotBlank String name, @NotBlank String password, String role) {}

    @PostMapping("/register")
    public ResponseEntity<?> register(@RequestBody RegisterRequest req) {
        if (userRepository.existsByUserId(req.userId())) return ResponseEntity.badRequest().body(Map.of("error","userId exists"));
        UserRole role = (req.role() == null) ? UserRole.USER : UserRole.valueOf(req.role().toUpperCase());
        User u = new User(req.userId(), req.name(), passwordEncoder.encode(req.password()), role);
        userRepository.save(u);
        return ResponseEntity.ok(Map.of("id", u.getId(), "userId", u.getUserId(), "name", u.getName(), "role", u.getRole()));
    }
    ...
    public ResponseEntity<?> login(@RequestBody LoginRequest req) {
        ...
        String token = jwtTokenProvider.createToken(u.getUserId(), u.getRole().name(), u.getName());
    ...
    public ResponseEntity<?> me(Principal principal) {
        ...
        User u = userRepository.findByUserId(principal.getName()).orElse(null);
