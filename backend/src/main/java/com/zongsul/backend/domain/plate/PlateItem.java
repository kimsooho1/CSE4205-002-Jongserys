...
/**
 * PlateItem 엔티티
 * - 한 식판(PlateResult)에 속한 개별 음식 항목과 남김율(0.0~1.0) 표현
 *
 * 매핑 상세
 * 1) @ManyToOne(fetch = LAZY)
 *    - 다:1 구조로 PlateResult에 연결. 지연 로딩을 통해 필요 시에만 부모 로딩
 * 2) remainingRatio
 *    - 0.0(하나도 안 남김) ~ 1.0(모두 남김) 범위 가정. DB 타입은 DOUBLE
 *    - 비즈니스 검증은 서비스/컨트롤러 단에서 수행 가능
 * 3) food
