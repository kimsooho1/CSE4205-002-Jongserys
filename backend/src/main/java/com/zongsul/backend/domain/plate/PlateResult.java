...
/**
 * PlateResult 엔티티
 * - 한 명(한 식판)의 AI 분석 결과를 표현하는 루트 엔티티
 * - plate_result 테이블과 매핑되며, 1:N으로 PlateItem(음식 단위 남김율)과 연관됨
 *
 * 주요 설계 포인트
 * 1) @OneToMany(mappedBy = "plateResult", cascade = ALL, orphanRemoval = true)
 *    - 부모(PlateResult) 저장 시 자식(PlateItem)도 함께 저장되도록 CASCADE 설정
 *    - 부모에서 자식 컬렉션에서 제거 시 DB에서도 삭제(orphanRemoval=true)
 *    - 실무에서 대량 업로드 시 컬렉션 조작 비용을 최소화하기 위해 영속성 전이로 단순화
 * 2) @PrePersist로 createdAt 자동 세팅
 *    - DB DEFAULT CURRENT_TIMESTAMP와 중복되지 않게 애플리케이션 차원에서도 보장
 * 3) 양방향 연관관계 편의 메서드 addItem
 *    - item.setPlateResult(this)와 items.add(item)를 함께 수행해 일관성 유지
 *
 * 주의사항
 * - 대규모 INSERT 성능이 중요하면 batch insert 설정(hibernate.jdbc.batch_size) 고려
 * - createdAt 기준 조회가 많으므로 인덱스(idx_plate_result_created_at)를 Flyway에서 생성
 */
@Entity
