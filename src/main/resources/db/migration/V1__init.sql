-- ============================================
-- Schema initialization for MySQL (Flyway-safe)
-- ============================================
...
-- 인덱스 생성 (이미 존재하면 경고만 발생하고 통과됨)
CREATE INDEX idx_plate_result_created_at ON plate_result(created_at);
CREATE INDEX idx_plate_item_food ON plate_item(food);

-- Distribution session and claim
...
CREATE INDEX idx_distribution_session_active ON distribution_session(active);
...
CREATE INDEX idx_menu_rec_week_day ON menu_recommendation(week_start_date, day_of_week);
